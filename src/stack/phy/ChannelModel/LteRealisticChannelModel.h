...
protected:
    ...
    //Calculate RSSI for D2D broadcast peers
    std::pair<Band,std::vector<double>> rssiBands;
    std::vector<std::pair<Band,double>> rssiVectorsBands;
    std::vector<std::vector<std::pair<Band,double>>> rssiD2DPeers;
...
public:
    ...
    std::vector<std::vector<std::pair<Band,double>>> getRSSI(LteAirFrame *frame, UserControlInfo* lteInfo_1, MacNodeId destId, Coord destCoord, MacNodeId enbId);
...
protected:
    ...
    bool computeInCellD2DInterference(MacNodeId eNbId, MacNodeId senderId, Coord senderCoord, MacNodeId destId, Coord destCoord, bool isCqi,
