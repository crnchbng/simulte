...
{
    EV<<"LteRealisticChannelModel::getAttenuation_D2D"<<endl;

    double movement = .0;
    double speed = .0;


    double sqrDistance = phy_->getCoord().distance(coord_2);

    speed = computeSpeed(nodeId, coord);
    EV<<"Correlation distance: "<<correlationDistance_<<endl;
        //If traveled distance is greater than correlation distance UE could have changed its state and
    // its visibility from eNodeb, hence it is correct to recompute the los probability
    if (movement > correlationDistance_
            || losMap_.find(nodeId) == losMap_.end())
    {
        EV<<"Distance: "<<sqrDistance<< "Scenario: "<<scenario_<<endl;
        computeLosProbability(sqrDistance, nodeId);
    }
...
std::vector<double> LteRealisticChannelModel::getRSRP_D2D(UserControlInfo* lteInfo_1, MacNodeId destId, Coord destCoord)
{
    ...
    EV<<"Source Coord: "<<sourceCoord<< endl;
    double antennaGainTx = 0.0;
    double antennaGainRx = 0.0;
    double noiseFigure = 0.0;
    double speed = 0.0;
    ...
    throw cRuntimeError("LteRealisticChannelModel::getRSRP_D2D");
    // attenuation for the desired signal
...
void LteRealisticChannelModel::computeLosProbability(double d,
        MacNodeId nodeId)
{
    EV<<"LteRealisticChannelModel::computeLosProbability"<<endl;
...
LteRealisticChannelModel::JakesFadingMap * LteRealisticChannelModel::obtainUeJakesMap(MacNodeId id)
{
    ...
    EV<<"LteRealisticChannelModel::obtainUeJakesMap nodeId: "<<id<<endl;
...
std::vector<std::vector<std::pair<Band,double>>> LteRealisticChannelModel::getRSSI(LteAirFrame *frame, UserControlInfo* lteInfo_1, MacNodeId destId, Coord destCoord, MacNodeId enbId)
{
    rssiD2DPeers.clear();
    inet::Coord sourceCoord = phy_->getCoord();
    std::vector<double> rssiVector;
    std::map<MacNodeId,inet::Coord> BroadcastueMap = binder_->getBroadcastUeInfo();
    std::map<MacNodeId,inet::Coord> UeInfoMap;
    std::map<MacNodeId,inet::Coord> ::iterator itb = BroadcastueMap.begin();

    //Check if the node is still present in the simulation
    std::vector<UeInfo*>* ueList_ = binder_->getUeList();
    std::vector<UeInfo*>::iterator position = ueList_->begin();


   if (BroadcastueMap.size()>0)
    {
        EV<<"Broadcast group size 1: "<<binder_->BroadcastUeInfo.size()<<endl;
        for(; itb != BroadcastueMap.end(); ++itb)
        {
           EV<<"Broadcast peers: "<<"Node Id: "<<itb->first <<"Coordinates: "<<itb->second<<endl;
         MacNodeId  nodeId = itb->first;
          inet::Coord coord = itb->second;
          EV<<"Node Id: "<<nodeId<<""<<"Coord: "<<coord<<endl;
           UeInfoMap[nodeId] = coord;
        }
    }
    else if (BroadcastueMap.size()==0)
    {
        MacNodeId  nodeId = lteInfo_1->getSourceId();
         inet::Coord coord = sourceCoord;
        UeInfoMap[nodeId]=coord;
    }

    else
    {
        throw cRuntimeError("Invalid UE in simulation");
    }

    EV<<"UEInfoMap size: "<<UeInfoMap.size()<<endl;
    std::map<MacNodeId,inet::Coord>::iterator itu = UeInfoMap.begin();

    if (UeInfoMap.size()!=0)
    {
    for(; itu != UeInfoMap.end(); ++itu)
    {

        AttenuationVector::iterator it;
        // Get Tx power
        double recvPower = 26; // dBm
        EV<<"recvpower: "<<recvPower<<endl;
        double antennaGainTx = 0.0;
        double antennaGainRx = 0.0;
        double noiseFigure = 0.0;
        double speed = 0.0;
        ...
        // Compute speed
        speed = computeSpeed(itu->first, itu->second);
        EV<<"speed: "<<speed<<endl;
        ...
        EV << "LteRealisticChannelModel::getRSSI - srcId=" << sourceId
                ...
                << endl
                << " - txPwr " << recvPower
                << " - ue1_Coord[" << sourceCoord << "] - ue2_Coord[" << itu->second<< "] - ue1_Id[" << sourceId << "] - ue2_Id[" << itu->first << "]" <<
                endl;
        //=================== END PARAMETERS SETUP =======================
        ...
        EV << "\t using parameters - noiseFigure=" << noiseFigure << " - antennaGainTx=" << antennaGainTx << " - antennaGainRx=" << antennaGainRx <<
                " - txPwr=" << recvPower << " - for ueId=" << sourceId << endl;
        EV<<"Fading enabled: "<<fading_<< ""<< "Fading type: "<<fadingType_ <<endl;
        // attenuation for the desired signal
        double attenuation = getAttenuation_D2D(sourceId, dir, sourceCoord, itu->first, itu->second); // dB
        ...
        for (unsigned int i = 0; i < band_; i++)
        {
            ...
            // add fading contribution to the received pwr
            double finalRecvPower = recvPower + fadingAttenuation; // (dBm+dB)=dBm
            EV<<"Final received power: "<<finalRecvPower<<endl;
            //if txmode is multi user the tx power is dived by the number of paired user
            // in db divede by 2 means -3db
            if (lteInfo_1->getTxMode() == MULTI_USER)
            {
        ...
        {
            computeInCellD2DInterference(enbId, sourceId, sourceCoord, itu->first, itu->second, (lteInfo_1->getFrameType() == FEEDBACKPKT), &inCellInterference,dir);
        }
        ...
        if( enableD2DInCellInterference_ && dir==D2D  )
        {
            ...
            // denominator expressed in dBm as (N+extCell+inCell)
            double den;
            EV << "LteRealisticChannelModel::getSINR - distance from my Peer = " << itu->second.distance(sourceCoord) << " - DIR=" << dirToA(dir)  << endl;

            // Add interference for each band
            for ( int i = 0; i < band_; i++)
            {
                //               (      mW            +  mW  +        mW            )
        ...
        else
        {
            for (int i = 0; i < band_; i++)
            {
                ...
                rssiVectorsBands.emplace_back(i,(noiseFigure + thermalNoise_));
                EV<<"RSSI bands: "<< rssiVector[i]<<endl;
                EV << "LteRealisticChannelModel::getRSSI - distance from my Peer = " << itu->second.distance(sourceCoord) << " - DIR=" << dirToA(dir) << " - rssi[" << rssiVector[i] << "]\n";
            }
        }

        //sender is a UE
        updatePositionHistory(sourceId, sourceCoord);
        rssiD2DPeers.push_back(rssiVectorsBands);
...
void LteRealisticChannelModel::setBand( unsigned int band )
{
    band_ = band ;
}

void LteRealisticChannelModel::setPhy( LtePhyBase * phy )
{
    phy_ = phy ;
}
